use crate::errors::{EvmSubsystemError, EvmSubsystemErrors, InterfaceError};
use crate::evm_stack::EvmStack;
use crate::gas::{gas_utils, Gas};
use crate::interpreter::CreateScheme;
use crate::{BytecodePreprocessingData, Interpreter, THIS_EE_TYPE};
use alloc::boxed::Box;
use core::any::Any;
use core::fmt::Write;
...
impl<'ee, S: EthereumLikeTypes> ExecutionEnvironment<'ee, S, EvmSubsystemErrors>
    for Interpreter<'ee, S>
{
    const NEEDS_SCRATCH_SPACE: bool = false;

    const EE_VERSION_BYTE: u8 = ExecutionEnvironmentType::EVM_EE_BYTE;

    type UsageError = <EvmSubsystemErrors as zk_ee::system::errors::SubsystemErrorTypes>::Interface;
    ...
    fn start_executing_frame<'a, 'i: 'ee, 'h: 'ee>(
        ...
        if scratch_space_len != 0 || decommitted_bytecode.len() != bytecode_len as usize {
            return Err(SubsystemError::Usage(InterfaceError::BytecodeNoPadding));
        }
        ...
        match modifier {
            ...
            a => return Err(SubsystemError::Usage(InterfaceError::UnexpectedModifier(a))),
        }
        ...
        S::IO: IOSubsystemExt,
    {
        ...
        let Some(ee_specific_deployment_processing_data) = ee_specific_deployment_processing_data
            ...
            return Err(SubsystemError::Usage(InterfaceError::NoDeploymentScheme));
        };
        let Ok(scheme) = <CreateScheme as EEDeploymentExtraParameters<S>>::from_box_dyn(
            ee_specific_deployment_processing_data,
        ) else {
            return Err(SubsystemError::Usage(InterfaceError::UnknownDeploymentData));
        };
