...
use core::fmt::Write;
use core::ops::Range;
use error::{EvmSubsystemError, EvmSystemWideError};
...
impl<'ee, S: EthereumLikeTypes> Interpreter<'ee, S> {
    ...
    ) -> Result<ExitCode, EvmSystemWideError> {
        let mut cycles = 0;
        let result = loop {
...
pub fn convert_exit_code_to_error(exit_code: ExitCode) -> Option<EvmSystemWideError> {
    if let
        ...
        Some(EvmSystemWideError::External(public_error))
    }
}
//
//
