...
impl<S: EthereumLikeTypes> Interpreter<'_, S> {
    pub fn lt(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, LT_NATIVE_COST)?;
    ...
    pub fn gt(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, GT_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
        if op1.gt(op2) {
            U256::write_one(op2);
        } else {
    ...
    pub fn slt(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, SLT_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn sgt(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, SGT_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn eq(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, EQ_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
        if op1.eq(op2) {
            U256::write_one(op2);
        } else {
    ...
    pub fn iszero(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, ISZERO_NATIVE_COST)?;
    ...
    pub fn bitand(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, AND_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn bitor(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, OR_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn bitxor(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, XOR_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn not(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, NOT_NATIVE_COST)?;
    ...
    pub fn byte(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, BYTE_NATIVE_COST)?;
    ...
    pub fn shl(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, SHL_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
    ...
    pub fn shr(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, SHR_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
        match op1.try_to_usize() {
            None => U256::write_zero(op2),
            Some(shift) => {
    ...
    pub fn sar(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::VERYLOW, SAR_NATIVE_COST)?;
        let (op1, op2) = self.stack.pop_1_and_peek_mut()?;
        apply_sar(op1, op2);
        Ok(())
    }
    ...
    pub fn clz(&mut self) -> InstructionResult {
        self.gas
            .spend_step_gas_and_native(gas_constants::LOW, CLZ_NATIVE_COST)?;
