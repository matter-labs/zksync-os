...
impl serde::Serialize for Bytes32 {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        // Always serialize as a fixed-size byte array [u8; 32].
        // This works for both human-readable (JSON: array of numbers) and
        // binary (bincode: raw 32 bytes) formats without pulling in
        // core::fmt machinery that generates MULH on riscv32.
        <[u8; 32] as serde::Serialize>::serialize(self.as_u8_array_ref(), serializer)
    }
}
...
mod serde_tests {
    ...
    fn bytes32_serde_roundtrip() {
        ...
        let encoded = AirbenderCodecV1::encode(&b32).unwrap();
        let decoded: Bytes32 = AirbenderCodecV1::decode(&encoded).unwrap();
