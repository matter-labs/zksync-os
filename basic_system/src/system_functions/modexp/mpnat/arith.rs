...
use super::{double::U512, mpnat::{MPNatU256, BASE, WORD_BITS}, U256};
use alloc::vec::Vec;
...
pub fn big_wrapping_mul<A: Allocator + Clone>(x: &MPNatU256<A>, y: &MPNatU256<A>, out: &mut [U256]) {
    let s = out.len();
    for i in 0..s {
        ...
        for j in 0..(s - i) {
            let (prod, carry) = shifted_carrying_mul(
                &out[i + j],
                x.digits.get(j).cloned().unwrap_or(U256::ZERO),
...
pub fn shifted_carrying_mul(a: &U256, x: U256, y: &U256, c: &U256) -> (U256, U256) {

    let mut r = U512::from_narrow_mul(x, y);
...
pub fn carrying_add(mut x: U256, y: U256, carry: bool) -> (U256, bool) {
    let carry = x.overflowing_add_assign_with_carry_propagation(&y, carry);
    (x, carry)
}
//
...
pub fn join_as_double(hi: U256, lo: U256) -> U512 {
    U512::from_words(lo, hi)
}
