...
// pub type Word = super::U256;
// #[cfg(target_pointer_width = "64")]
// pub type DoubleWord = u128;

// pub const WORD_BYTES: usize = core::mem::size_of::<Word>();
pub const WORD_BITS: usize = U256::BYTES * 8;
pub const BASE: U512 = U512::ONE;
...
impl<A: Allocator + Clone> MPNatU256<A> {
    ...
    //     };
    //     // rewritten without grow usage on alloc for zk os
    ...
    // pub(crate) fn sub_to_same_size(&mut self, other: &Self) {
    //     // Remove leading zeros before starting
    //     while self.digits.len() > 1 && self.digits.last() == Some(&U256::ZERO) {
    //         self.digits.pop();
    //     }
    //
    ...
    //     }
    //
    //     let other_most_sig = other.digits.last().unwrap().to_double();
    //
    ...
    //     let shift = other_most_sig.low().leading_zeroes() as u32;
    //     if shift > 0 {
    //         // Normalize self
    ...
    //         debug_assert_eq!(
    //             normalized[n - 1].leading_zeroes(),
    //             0,
    //             "Most significant bit is set"
    //         );
    //
    //         // Run algorithm on normalized values
    //         self.sub_to_same_size(&MPNatU256 { digits: normalized });
    //
    ...
    //     }
    //
    //     let other_second_sig = other.digits[n - 2].to_double();
    //     let mut self_most_sig = U256::ZERO;
    //     for j in (0..=m).rev() {
    ...
    //             let b = join_as_double(r_hat as U256, self_third_sig);
    //             if q_hat >= BASE || a > b {
    //                 q_hat -= 1;
    ...
    //         }
    //
    //         let mut borrow = in_place_mul_sub(&mut self.digits[j..], &other.digits, q_hat as U256);
