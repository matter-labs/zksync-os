...
pub struct EthereumMPT<'a, A: Allocator + Clone> {
    ...
    pub(crate) extension_nodes:
        BidirectionalMap<ShortNodeIndex<'a>, (&'a [u8], ExtensionNode<'a>), A>,
                ...
                self.branch_nodes.get_mut(&short_index)
            {
                ...
                let branches_encodings_concatenation = unsafe {
                    core::slice::from_ptr_range(
                        pieces[0].full_encoding().as_ptr()
                            ..pieces[15].full_encoding().as_ptr_range().end,
                    )
                };
            ...
            'inner: loop {
                match self.descend_with_proof(&mut path, key, layer, parent_node, interner)? {
                    ProofPath::Diverged { allocated_node } => {
                        self.link_if_needed(parent_node, parent_branch_index, allocated_node)?;
