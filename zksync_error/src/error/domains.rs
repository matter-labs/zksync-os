...
impl ZksyncError {
    pub fn get_kind(&self) -> crate::kind::Kind {
        match self {
            ...
            ZksyncError::Os(Os::EVM(_)) => Kind::Os(OsCode::EVM),
    ...
    pub fn get_code(&self) -> u32 {
        match self {
            ...
            ZksyncError::Os(Os::EVM(error)) => Into::<EVMCode>::into(error) as u32,
...
impl ICustomError<ZksyncError, ZksyncError> for EVM {
    fn to_unified(&self) -> ZksyncError {
        Os::EVM(self.clone()).to_unified()
    }
}
impl From<EVM> for Os {
...
impl crate::documentation::Documented for Os {
    ...
    fn get_documentation(
        ...
        match self {
            Os::EVM(error) => error.get_documentation(),
...
impl std::fmt::Display for Os {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Os::EVM(component) => component.fmt(f),
