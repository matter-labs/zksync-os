...
#[non_exhaustive]
pub enum EVM {
    InvalidArguments { details: String } = 1u32,
    GenericError { message: String } = 0u32,
}
impl std::error::Error for EVM {}
...
impl CustomErrorMessage for EVM {
    fn get_message(&self) -> String {
        match self {
            EVM::InvalidArguments { details } => {
                format!("[os-evm-1] Invalid arguments: {details}.")
            }
            EVM::GenericError { message } => {
                format!("[os-evm-0] Generic error: {message}")
            }
        }
    }
}
...
pub enum ExecutionEnvironments {
    Evm { inner: Box<EVM> } = 1u32,
    Wasm = 2u32,
    GenericError { message: String } = 0u32,
}
impl std::error::Error for ExecutionEnvironments {}
...
impl std::fmt::Display for ExecutionEnvironments {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.get_message())
    }
}
...
impl CustomErrorMessage for ExecutionEnvironments {
    fn get_message(&self) -> String {
        match self {
            ExecutionEnvironments::Evm { inner } => {
                format!("[os-ee-1] EVM execution environment error: {inner}.")
            }
            ExecutionEnvironments::Wasm => {
                format!("[os-ee-2] WASM execution environment error.")
            }
...
impl std::error::Error for Storage {}
    ...
    fn get_message(&self) -> String {
        match self {
            Storage::GenericError { message } => {
                format!("[os-store-0] Generic error: {message}")
            }
        }
    }
}
