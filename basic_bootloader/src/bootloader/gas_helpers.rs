...
pub fn get_resources_for_tx<S: EthereumLikeTypes>(
    ...
    // Charge for calldata
    let calldata_gas = gas_cost_for_calldata(calldata)?;
...
///
/// Computes the gas cost for the transaction's calldata.
///
pub fn gas_cost_for_calldata(calldata: &[u8]) -> Result<usize, InternalError> {
