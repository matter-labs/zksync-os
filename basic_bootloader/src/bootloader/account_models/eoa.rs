    ...
    S::IO: IOSubsystemExt,
{
    fn validate(
            ...
            .get_nominal_token_balance(caller_ee_type, resources, &from)
        {
            ...
            Err(SystemError::Internal(e)) => return Err(TxError::BootloaderSubsystemError(e.into())),
        }
    ...
    fn charge_additional_intrinsic_gas(
        ...
        if is_deployment {
            ...
            match resources.charge(&S::Resources::from_ergs(ergs_to_spend)) {
                ...
                Err(SystemError::Internal(e)) => return Err(BootloaderSubsystemError::Defect(e).into()),
            };
        }
        Ok(())
    }
