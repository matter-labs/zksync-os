...
fn write_fq<D: zk_ee::common_traits::TryExtend<u8> + ?Sized>(el: Fq, output: &mut D) {
    // TODO: propagate errors?
    output.try_extend([0u8; 16]).map_err(|_| ()).unwrap();
    // BE
    for word in el.into_bigint().0[..6].iter().rev() {
        output
            .try_extend(word.to_be_bytes())
            .map_err(|_| ())
            .unwrap();
    }
}
...
fn write_g1<D: zk_ee::common_traits::TryExtend<u8> + ?Sized>(el: G1Affine, output: &mut D) {
    if let Some((x, y)) = el.xy() {
        ...
        // all zeroes
        output
            .try_extend([0u8; G1_SERIALIZATION_LEN])
            .map_err(|_| ())
            .unwrap();
    }
}
...
fn write_g2<D: zk_ee::common_traits::TryExtend<u8> + ?Sized>(el: G2Affine, output: &mut D) {
    if let Some((x, y)) = el.xy() {
        ...
        // all zeroes
        output
            .try_extend([0u8; G2_SERIALIZATION_LEN])
            .map_err(|_| ())
            .unwrap();
    }
}
