...
fn run_inner(
    ...
    #[cfg(feature = "cycle_marker")]
    {
        ...
        // `airbender_host::CycleMarker` upstream — but cargo treats
        // our pinned `zksync-airbender?rev=...` source and
        // airbender-platform's transitive `zksync-airbender?branch=dev`
        // source as distinct packages even when they resolve to the
        // same SHA, so the `From` impl resolves against
        // airbender-platform's copy of the type, not ours. Convert by
        // hand — both structs have all-public fields and the shape
        // mirrors upstream's `From` impl.
