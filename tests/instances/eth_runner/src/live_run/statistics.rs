...
pub fn log_run_statistics(
    ...
    info!(
        "Blocks in range: {} ({} to {})",
        blocks_in_range as u64, start_block, end_block
    );
    info!(
        "Blocks actually processed: {}",
    ...
    info!(
        "Blocks skipped (other): {}",
        blocks_in_range as u64
            - stats.blocks_actually_processed
            - stats.blocks_skipped_already_succeeded
            - stats.blocks_skipped_trace_fetch
    );
    info!(
        "Failures: {} ({} critical)",
        stats.failures, stats.critical_failures
    );
    info!("");
    info!("=== Timing Breakdown ===");
    info!(
        "  Initialization:      {:6.2}ms ({:5.1}%)",
        init_time.as_secs_f64() * 1000.0,
        init_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
    );
    if stats.blocks_actually_processed > 0 {
        info!(
            "  Block execution:      {:6.2}ms ({:5.1}%)",
            stats.total_block_time.as_secs_f64() * 1000.0,
            stats.total_block_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
        );
        info!(
            "  Per-block overhead:  {:6.2}ms ({:5.1}%)",
            stats.total_overhead_time.as_secs_f64() * 1000.0,
            stats.total_overhead_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
        );
        info!("    (webhooks, error handling)");
        if stats.total_prefetch_time.as_secs_f64() > 0.0 {
            info!(
                "  Prefetching:          {:6.2}ms ({:5.1}%)",
                stats.total_prefetch_time.as_secs_f64() * 1000.0,
                stats.total_prefetch_time.as_secs_f64() / total_time.as_secs_f64() * 100.0
            );
            info!(
                "    Blocks prefetched: {} (avg {:.2}ms per block)",
                stats.total_blocks_prefetched,
                stats.total_prefetch_time.as_secs_f64() * 1000.0
                    / stats.total_blocks_prefetched.max(1) as f64
            );
        }
        info!(
            "  Other overhead:      {:6.2}ms ({:5.1}%)",
            total_overhead_other.as_secs_f64() * 1000.0,
            total_overhead_other.as_secs_f64() / total_time.as_secs_f64() * 100.0
        );
        info!(
            "  Total:               {:6.2}ms ({:5.1}%)",
        ...
        info!("");
        let avg_time_per_block =
            stats.total_block_time.as_secs_f64() / stats.blocks_actually_processed as f64;
        let avg_total_per_block = total_time.as_secs_f64() / stats.blocks_actually_processed as f64;
        info!("=== Per Block Averages ===");
        info!(
            "  Execution time:      {:.2}ms",
            avg_time_per_block * 1000.0
        );
        info!(
            "  Total time (w/ overhead): {:.2}ms",
            avg_total_per_block * 1000.0
        );
        info!(
            "  Blocks per second:  {:.2}",
            stats.blocks_actually_processed as f64 / total_time.as_secs_f64()
        );
        if stats.prefetch_hits + stats.prefetch_misses > 0 {
            let prefetch_hit_rate = stats.prefetch_hits as f64
            ...
            info!(
                ...
                100.0 - prefetch_hit_rate
            );
            info!(
                "  Total blocks prefetched: {}",
                stats.total_blocks_prefetched
            );
        }
    }
